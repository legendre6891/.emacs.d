...
*** F-keys
**** F2 and F3
Use =F2= and =F3= to highlight symbols.
#+BEGIN_SRC emacs-lisp
  (global-set-key (kbd "<f3>") #'highlight-phrase)
  (global-set-key (kbd "<f4>") #'unhighlight-regexp)
#+END_SRC
**** F8
...
Disabled for now! Ido may still be better ...
#+BEGIN_SRC emacs-lisp
  (use-package helm
      :ensure t
      :init
...
Fuzzy match is a must.
#+BEGIN_SRC emacs-lisp
  (setq helm-buffers-fuzzy-matching t)
  (setq helm-recentf-fuzzy-match t)
#+END_SRC

Other stuff as introduced by =tudho=. 
#+BEGIN_SRC emacs-lisp
  (setq
...
** Key-combo
Key-combo is like key-chord on steroids. Useful for some LaTeX addons.
EDIT: made my own key-combo, so let's disable this for now.
#+BEGIN_SRC emacs-lisp  :tangle no
...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (if (eq system-type 'darwin)
      (my-set-face "InconsolataGo" 14))

  (if (eq system-type 'windows-nt)
      (my-set-face "Consolas" 12))

  (if (eq system-type 'gnu/linux)
      (my-set-face "PragmataPro Mono" 12))
#+END_SRC
...
default mode line. 
*Actually, now I decided against it. It serves no real purpose.*
#+BEGIN_SRC emacs-lisp  :tangle no
  (use-package smart-mode-line
      ...
      :config
      (progn
        (add-to-list 'sml/replacer-regexp-list '("^~/Dropbox (MIT)/" ":DB:") t)
        (sml/setup)))
#+END_SRC
...
** Smooth scroll
Appparently this package is pretty good! [Update: not worth it.]
#+BEGIN_SRC emacs-lisp :tangle no
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun setup-yasnippet-next-key (key)
    ...
    (define-key yas-keymap key 'yas-next-field-or-maybe-expand))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (add-hook 'plain-TeX-mode-hook (↑ (setup-yasnippet-next-key (kbd "<C-tab>"))))
  (add-hook 'LaTeX-mode-hook (↑ (setup-yasnippet-next-key (kbd "<C-tab>"))))
